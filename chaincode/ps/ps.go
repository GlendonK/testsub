...
/*
	This file was modified. NewKeyPair returns [][]bytes for private and pub key originally,
	and have been changed to []kyber.Scalar and []kyber.Point.
	Because Sign method receiving []kyber.Scalar and []kyber.Point for private and public key.
*/
...
func NewKeyPair(suite pairing.Suite, randoms []cipher.Stream) ([]kyber.Scalar, []kyber.Point, error) {
	var PriKey [][]byte
	var PubKey [][]byte
	...
	/*
		Convert []byte PriKey to []kyber.Scaler.
		Convert []byte PKey to []kyber.Point.
	*/
	var pubList []kyber.Point
	// var priList []kyber.Scalar
	...
	for i := range PriKey {
		...
		pub := suite.G2().Point().Mul(Pkey, nil)
	...
	for i := range PriKey {
		p := suite.G2().Scalar()
		p.UnmarshalBinary(PriKey[i])

		priList = append(priList, p)

	}

	return priList, pubList, nil
}

// Randomizability .Power a point to a random number
func power(binH []byte, h kyber.Point, suite pairing.Suite) ([]byte, kyber.Point) {

	// generate random number
	// b := big.NewInt(100)
	// r, _ := rand.Int(rand.Reader, b)

	// // the scaler used to multiply the point by
	// g := suite.G1().Scalar().Pick(random.New())

	// h = h.Mul(g, h)

	// for i := 0; i < int(r.Int64()); i++ {
	// 	h = h.Mul(g, h)
	// }

	// s2b, _ := h.MarshalBinary()

	// the scaler used to multiply the point by
	g := suite.G1().Scalar().Pick(random.New())

	g1 := suite.G1().Point().Mul(g, h)

	s2b, _ := g1.MarshalBinary()
...
func Sign(suite pairing.Suite, priKey []kyber.Scalar, msg []byte) ([][]byte, error) {
	...
	// power point h with random number
	binH, h = power(binH, h, suite)
	S = append(S, binH)
